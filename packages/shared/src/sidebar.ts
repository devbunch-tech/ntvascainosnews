...
const KNOWN = new Set<string>(SIDEBAR_WIDGETS.map((w) => w.key));

const labelOf = (key: string) => SIDEBAR_WIDGETS.find((w) => w.key === key)?.label ?? key;
 ...
 */
export function resolveSidebarWidgets(
  stored?: { key?: string | null; visible?: boolean | null }[] | null,
): SidebarWidget[] {
  const out: SidebarWidget[] = [];
...
export function resolveAdLimit(value?: number | null): number {
  if (typeof value !== "number" || !Number.isFinite(value)) return DEFAULT_AD_LIMIT;
