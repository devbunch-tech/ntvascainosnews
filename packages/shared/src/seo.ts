...
export function buildDescription(input: {
  ...
  const source =
    [input.subtitle, input.excerpt].map((v) => v?.trim()).find((v) => v && v.length > 40) ??
  ...
  if (lastStop > DESCRIPTION_MAX * 0.6) return cut.slice(0, lastStop + 1).trim();
...
function properNouns(title: string): string[] {
  const found: string[] = [];
  ...
  for (const match of title.matchAll(pattern)) {
    ...
    if (match.index === 0 && !match[2]) continue;
    found.push(value);
  }
  return found;
}
...
export function buildKeywords(input: {
  ...
  const push = (value: string) => {
    ...
    if (clean.length > 2 && !exists) out.push(clean);
  };

  for (const tag of input.tags ?? []) push(tag);
  if (input.category) push(input.category);
  for (const noun of properNouns(input.title)) push(noun);
  ...
  for (const brand of BRAND_KEYWORDS) push(brand);
...
export function buildPostSeo(
  ...
  manual?: { description?: string | null; keywords?: string[] | null; geo?: Partial<PostGeo> | null },
): PostSeo {
  return {
